...
+ (QRScanResultModel *)scanResultByImage:(UIImage *)image
{
    ...
    NSArray *results = (NSArray *)[imagePicker scanImage:image.CGImage];
    
    ZBarSymbol *symbol = results.firstObject;
    
    NSInteger codeCount = results.count;
    
    if(codeCount >= 1){
        if(codeCount > 1){
            resultModel.isMultipleResult = YES;
            resultModel.resultString = @"";//暂时不让支持多二维码
        }else{
            if(symbol.type == ZBAR_PDF417 || symbol.type == ZBAR_QRCODE){
                resultModel.resultType = QRScanResultType_QRCodeType;
            }
            else{//条形码
                resultModel.resultType = QRScanResultType_BarCodeType;
            }
            resultModel.resultString = symbol.data;
        }
        
    }
    else {//ZBar识别失败后再用ZXing试试
